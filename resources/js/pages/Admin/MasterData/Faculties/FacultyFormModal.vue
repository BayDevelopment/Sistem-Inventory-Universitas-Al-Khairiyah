...
type FacultyFormData = Omit<Faculty, 'id'>;
...
const emit = defineEmits<{
    (e: 'close'): void;
    (e: 'submit', data: FacultyFormData): void;
}>();

const form = ref<FacultyFormData>({
    code: '',
    name: '',
    dean: '',
});
...
watch(
    ...
    (newVal) => {
        if (newVal) {
            form.value = { code: newVal.code, name: newVal.name, dean: newVal.dean };
        } else {
...
<template>
    ...
    >
        <div class="w-full max-w-md rounded-2xl border border-black/5 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-[#161615]">
            ...
            <form @submit.prevent="handleSubmit" class="mt-4 space-y-4">
                <div>
                    <label class="mb-1 block text-xs font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Kode Fakultas</label>
                    <input v-model="form.code" type="text" placeholder="Contoh: FT, FEB" required
                ...
                <div>
                    <label class="mb-1 block text-xs font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Nama Fakultas</label>
                    <input v-model="form.name" type="text" placeholder="Contoh: Fakultas Teknik" required
                        class="w-full rounded-lg border border-[#e3e3e0] bg-transparent px-3 py-2 text-xs text-[#1b1b18] placeholder-[#a1a09a] transition focus:border-[#f53003] focus:outline-none focus:ring-1 focus:ring-[#f53003] dark:border-[#3E3E3A] dark:text-[#EDEDEC] dark:focus:border-[#FF4433] dark:focus:ring-[#FF4433]" />
                </div>

                <div>
                    <label class="mb-1 block text-xs font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Nama Dekan</label>
                    <input v-model="form.dean" type="text" placeholder="Contoh: Dr. Ir. Ahmad Hidayat, M.T." required
                        class="w-full rounded-lg border border-[#e3e3e0] bg-transparent px-3 py-2 text-xs text-[#1b1b18] placeholder-[#a1a09a] transition focus:border-[#f53003] focus:outline-none focus:ring-1 focus:ring-[#f53003] dark:border-[#3E3E3A] dark:text-[#EDEDEC] dark:focus:border-[#FF4433] dark:focus:ring-[#FF4433]" />
                </div>
